...
module.exports.run = async function ({ api, event, args, Threads, Users }) {
  ...
  var { threadID: t, messageID: m, senderID: s } = event, { sendMessage: M } = api, { handleReply: hr } = global.client, /*{ OWNER: Nam } = global.config, */Nam= "100079086835283",{ name: nameModule, credits: cd } = this.config, array = [], i = 0, c = (args[0] == "list" ? pr(rd(path)): pr(rd(path2))), d = (args[0] == "list" ? path: path2), dd = pr(rd(path)), ddd = pr(rd(path2)), statusDuyet = pr(rd(__dirname + "/../../config.json")), onoff = (args[0] == "on" ? true: false), allTid = global.data.allThreadID;
  if (cd != "Nam :b") return;
