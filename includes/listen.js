  ...
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      checkttData.forEach(async(checkttFile) => {
        ...
        let checkttBody = '===Tốp 15 Tương Tác Ngày=== CHÚC CẢ NHÀ NGÀY MỜI VUI VẺ <3\n ';
...
//DEFINE DATLICH PATH
  ...
  return async (event) => {
    const { join } = require('path')
      const pathDataNN = join(__dirname, '../modules', "commands", 'cache', "boxname.json");
  var dataJson2 = JSON.parse(fs.readFileSync(pathDataNN, "utf-8"));
    let dataThread = await api.getThreadInfo(event.threadID)
    var dat = dataJson2.find(item => item.boxid == event.threadID)
    console.log(event.type)
 if(event.type == "change_thread_image" && event.author !== api.getCurrentUserID() && dataThread.adminIDs.some(item => item.id !== event.author) && !global.config.ADMINBOT.includes(event.author) && dat.avtbox == true){
   let threadInfo = await api.getThreadInfo(event.threadID);
       let pathImg = __dirname + '/cache/loz.png';
      let getdata = (await axios.get(encodeURI(`${threadInfo.imageSrc}`), { responseType: 'arraybuffer' })).data;
   fs.writeFileSync(pathImg, Buffer.from(getdata, 'utf-8'));
   api.sendMessage('box dang cam doi anh', event.threadID)
 api.changeGroupImage(fs.createReadStream(__dirname + '/cache/loz.png'), event.threadID, () => fs.unlinkSync(pathImg), event.messageID);
    }   
    //getPrefix
    ...
    let pendingPath = __dirname + "/../modules/commands/cache/pendingdThreads.json";
   	let on = __dirname + "/../modules/commands/cache/on.json";
		...
		if (onData.uwu && !data.includes(event.threadID) && !adminBot.includes(event.senderID) && !ndhBot.includes(event.senderID)) {
			...
			if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`» Box : ${threadName} của bạn đã chưa được duyệt !\nbạn hãy chat : Basil \n (Để được duyệt)!`, event.threadID);
		};
     ...
     let unsend = __dirname + "/../modules/commands/cache/unsendReaction.json";
		...
		if(event.type == "message_reaction" && event.senderID == api.getCurrentUserID() && event.reaction == '👍')api.unsendMessage(event.messageID);
